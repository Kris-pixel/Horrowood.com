...
$query = "SELECT COUNT(*) FROM items WHERE id LIKE 'f-%'";
...
$query = "SELECT * FROM items 
            JOIN item_types ON items.type_code = item_types.code
...
if ($page > 1) {
    echo "<a class='pagination  pagination-active' href='http://horrowood.com/index.php?action=admin&tab=Film&page=1' class=''><<</a>";
    $prevpage = $page - 1;
    echo "<a class='pagination' href='http://horrowood.com/index.php?action=admin&tab=Film&page=$prevpage' class=''><</a>";
}
$range = 3;
for ($i = ($page - $range); $i < ($page + $range) + 1; $i++) {
    if (($i > 0) && ($i <= $totalpages)) {
        if ($i == $page) {
            ...
            echo "<a class='pagination'  href='http://horrowood.com/index.php?action=admin&tab=Film&page=$i'>$i</a>";
        }
    }
}
if ($page != $totalpages) {
    $nextpage = $page + 1;
    echo "<a class='pagination' href='http://horrowood.com/index.php?action=admin&tab=Film&page=$nextpage'>></a>";
